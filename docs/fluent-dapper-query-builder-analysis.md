...
- entity command convenience 只作为单表命令 builder 的便利层：这里的 entity 是作为值来源的已映射 CLR 对象，不是被跟踪的 ORM 实体；`Insert(entity)`、`Update(entity)`、`Delete(entity)` 是常用主入口，`Values(entity)` 和 `WhereKey(entity)` 是底层 builder convenience，最终仍通过 `ToCommand()` / `Execute()` / `ExecuteAsync()` 走 Dapper。
