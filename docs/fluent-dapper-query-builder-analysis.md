...
- entity command convenience 只作为单表命令 builder 的便利层：这里的 entity 是作为值来源的已映射 CLR 对象，不是被跟踪的 ORM 实体；`Values(entity)`、`Set(entity)`、`WhereKey(entity)` 从对象读取映射属性并生成显式 SQL 命令模型，最终仍通过 `ToCommand()` / `Execute()` / `ExecuteAsync()` 走 Dapper。
...
- `Value(...)`、`Values(entity)`、`Set(...)`、`Set(entity)`、`WhereKey(entity)`、命令 builder 上的 `Execute()`、`ExecuteAsync()`
- `Insert(entity)`、`Update(entity)`、`Delete(entity)` 短入口，等价于 SQL-shaped builder 组合
...
- 支持 `[Key]`、`Id` / `<TypeName>Id` 主键约定，用于 `WhereKey(entity)` 和短 `Update(entity)` / `Delete(entity)` 入口生成等值谓词；没有主键或复合主键时不会退化为整表操作。
- 支持 `[DatabaseGenerated(DatabaseGeneratedOption.Identity)]` 和 `[DatabaseGenerated(DatabaseGeneratedOption.Computed)]` 映射属性在 `Values(entity)` / `Insert(entity)` 中跳过；显式 `.Value(...)` 仍由调用者控制，Db4Net 不读取或跟踪数据库生成值。
