...
`[DatabaseGenerated(DatabaseGeneratedOption.Identity)]` and `[DatabaseGenerated(DatabaseGeneratedOption.Computed)]` mapped properties are omitted by `Values(entity)`, `Insert(entity)`, `InsertMany(entities)`, and conflict-aware insert values. Explicit `.Value(...)` calls remain caller-controlled.
