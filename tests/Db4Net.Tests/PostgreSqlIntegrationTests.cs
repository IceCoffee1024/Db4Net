...
public sealed class PostgreSqlIntegrationTests
{
    ...
    {
        var table = ExternalDatabaseTestSupport.CreateTableName("postgresql", "conflict_users");
        await using var connection = await OpenConnectionAsync();
        ...
        try
        {
            await ExecuteAsync(connection, $"""
                ...
                INSERT INTO "{table}" ("Id", "Name") VALUES (1, 'Alice'), (2, 'Bob');
                """);
            var db = connection.UseDb4Net(Db4NetOptions.PostgreSql);
    ...
    {
        var table = ExternalDatabaseTestSupport.CreateTableName("postgresql", "conflict_many");
        await using var connection = await OpenConnectionAsync();
    ...
    {
        var table = ExternalDatabaseTestSupport.CreateTableName("postgresql", "unique_users");
        await using var connection = await OpenConnectionAsync();

        try
        {
            await ExecuteAsync(connection, $"""
                CREATE TABLE "{table}" ("Id" integer PRIMARY KEY, "Email" text NOT NULL UNIQUE, "Name" text NOT NULL);
    ...
    {
        var table = ExternalDatabaseTestSupport.CreateTableName("postgresql", "generated_users");
        await using var connection = await OpenConnectionAsync();

        try
        {
            await ExecuteAsync(connection, $"""
                CREATE TABLE "{table}" ("Id" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, "Name" text NOT NULL);
                """);
            var db = connection.UseDb4Net(Db4NetOptions.PostgreSql);
