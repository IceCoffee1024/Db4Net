...
/// <summary>
/// Executes DELETE commands for multiple mapped CLR model instances using key properties for each WHERE clause.
/// </summary>
/// <typeparam name="T">The CLR model type used for table and member mapping.</typeparam>
public sealed class DeleteManyCommandBuilder<T>
{
    ...
    internal DeleteManyCommandBuilder(Db4NetOptions options, IDbConnection? connection, string table, IEnumerable<T> entities, Db4NetExecutionOptions? executionOptions = null)
    {
        _options = options;
    ...
    public IReadOnlyList<RenderedSqlCommand> ToCommands()
    {
        ...
        return _entities.Select(entity => renderer.RenderDelete(_table, entity)).ToArray();
    }

    /// <summary>
    /// Executes the DELETE command once for each entity through Dapper and returns the affected row count.
    /// </summary>
    ...
    public int Execute(Db4NetExecutionOptions? options = null)
    {
        ...
        var sql = new ManyCommandSqlRenderer(_options.Dialect).RenderDeleteTemplate<T>(_table);
        return _executor.Execute(sql, _entities, options);
    }

    /// <summary>
    /// Asynchronously executes the DELETE command once for each entity through Dapper and returns the affected row count.
    /// </summary>
    /// <param name="options">Optional Dapper execution settings such as transaction, timeout, or command type.</param>
